...
func TestBasicCollection_FindAllByBaseIDs(t *testing.T) {
	mt := mtest.New(t, mtest.NewOptions().ClientType(mtest.Mock))
	...
	mt.Run("successful find", func(mt *mtest.T) {
		...
		mt.AddMockResponses(mtest.CreateCursorResponse(1, "DBName.CollectionName", mtest.FirstBatch, bson.D{
			...
			mtest.CreateCursorResponse(0, "DBName.CollectionName", mtest.NextBatch),
		)

		result, err := basicCollection.FindAllByBaseIDs(context.Background(), []string{"base1"})
		assert.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, "base1", result[0].BaseID)
...
func TestBasicCollection_FindAllByUserID(t *testing.T) {
	mt := mtest.New(t, mtest.NewOptions().ClientType(mtest.Mock))
	...
	mt.Run("successful find", func(mt *mtest.T) {
		...
		result, err := basicCollection.FindAllByUserID(context.Background(), "user1")
		assert.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, "base1", result[0].BaseID)
		assert.Equal(t, "Test", result[0].Name)
...
func TestBasicCollection_SoftDeleteByBoardID(t *testing.T) {
	mt := mtest.New(t, mtest.NewOptions().ClientType(mtest.Mock))
	...
	mt.Run("successful soft delete", func(mt *mtest.T) {
		...
		deletedCount, err := basicCollection.SoftDeleteByBoardID(context.Background(), boardID)
		assert.NoError(t, err)
		assert.Equal(t, 1, deletedCount)
	})
	...
	mt.Run("soft delete failure", func(mt *mtest.T) {
		...
		deletedCount, err := basicCollection.SoftDeleteByBaseID(context.Background(), boardID)
		assert.Error(t, err)
		assert.Equal(t, 0, deletedCount)
	})
}
